...
import (
	...
	"github.com/julieqiu/exp/surfer/internal/commandyaml"
...
func parseGcloudYAML(path string) (*gcloudconfig.Config, error) {
	data, err := os.ReadFile(path)
	if err != nil {
		return nil, fmt.Errorf("failed to read file: %w", err)
	}

	var cfg gcloudconfig.Config
...
func generateCommands(model *api.API, cfg *gcloudconfig.Config, outputDir string) error {
	...
	for _, service := range validServices {
		...
		for _, method := range service.Methods {
			...
			cmdName := commandyaml.DeriveCommandName(method.Name)
...
func generateCommandFiles(resourceDir, partialsDir, cmdName string, method *api.Method, cfg *gcloudconfig.Config) error {
	...
	for _, track := range apiCfg.ReleaseTracks {
		...
		cmd := commandyaml.GenerateCommand(method, apiCfg, track)
...
func findAPIConfig(cfg *gcloudconfig.Config, method *api.Method) *gcloudconfig.API {
