...
release:
  ...
  tag_format: '{id}/v{version}' # Tag format (optional)
```

Repository-level configuration in `.librarian/config.yaml`:

```yaml
release:
  tag_format: '{name}-v{version}' # Default tag format for all libraries
```
